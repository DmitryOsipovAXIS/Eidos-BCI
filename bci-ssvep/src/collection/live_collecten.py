...
def _draw_inference_box(
    ...
    if scores:
        parts = [f"L:{scores[0]:.3f}", f"R:{scores[1]:.3f}"] if len(
            scores) >= 2 else []
...
def _draw_single(
    ...
    screen.blit(fmed.render(f"FPS {fps:0.1f}", True, GREY), fmed.render(
        f"FPS {fps:0.1f}", True, GREY).get_rect(center=(screen.get_width() - 90, 26)))
...
def _draw_dual(
    ...
    _draw_text_center(screen, fbig, "LOOK LEFT" if label ==
...
def _start_menu(fullscreen: bool) -> dict:
    ...
    W, H = (info.current_w, info.current_h) if fullscreen else (
        int(info.current_w * 0.9), int(info.current_h * 0.9))
    ...
    while selected is None:
        ...
        for k, text, _ in options:
            ...
            y += 46
        _draw_text_center(
            screen, fsml, "Press 1-5 or click an option (click to focus)", H - 90, GREY)
...
def _run_single(
    ...
    pygame.init()
    info = pygame.display.Info()
    W, H = (info.current_w, info.current_h) if args.fullscreen else (
        int(info.current_w * 0.9), int(info.current_h * 0.9))
    ...
    cy = H // 2
    rect = pygame.Rect(int(W * 0.12) if side ==
                       "left" else int(W * 0.53), cy - bh // 2, bw, bh)
...
def _run_alternating(
    ...
    for idx, label in enumerate(labels):
        ...
        if stream is not None:
            rec_thread = threading.Thread(
                target=_record_window,
                args=(stream, label, alt_block_s,
                      args.discard, start_event, result),
                daemon=True,
            )
            rec_thread.start()
...
def start_live(args: LiveCollectArgs | None = None, broadcast=None, loop=None) -> None:
    ...
    if args.mode == "single":
        side = selection.get(
            "side", args.single_side) if selection else args.single_side
        hz = selection.get(
            "hz", args.single_hz) if selection else args.single_hz
        duration_s = selection.get(
            "duration", args.single_duration) if selection else args.single_duration
        ...
        left_hz = selection.get(
            "left_hz", args.left_hz) if selection else args.left_hz
        right_hz = selection.get(
            "right_hz", args.right_hz) if selection else args.right_hz
        alt_total_s = selection.get(
            "alt_total", args.alt_total) if selection else args.alt_total
        alt_block_s = selection.get(
            "alt_block", args.alt_block) if selection else args.alt_block
        both_flicker = selection.get(
            "both_flicker", args.both_flicker) if selection else args.both_flicker
    ...
    if args.mode == "alt":
        ...
        if not eeg_list:
            print("Start before broadcast alt", flush=True)
            if broadcast is None:
                print("Broadcast is None so broadcast won't work", flush=True)
            if broadcast is None:
                print("Loop is None so broadcast won't work", flush=True)

            if broadcast is not None and loop is not None:
                asyncio.run_coroutine_threadsafe(
                    broadcast("No EEG recorded."), loop)
    ...
    if eeg is None:
        print("Start before broadcast", flush=True)
        if broadcast is None:
            print("Broadcast is None so broadcast won't work", flush=True)
        if broadcast is None:
            print("Loop is None so broadcast won't work", flush=True)
