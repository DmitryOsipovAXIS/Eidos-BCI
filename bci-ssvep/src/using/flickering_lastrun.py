...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Code\\School\\3de\\Sem 2\\Bachelor Project\\AIandData\\bci-ssvep\\src\\collection\\flickering_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    stimuli1 = visual.Rect(
        win=win, name='stimuli1',
        width=(0.5, 0.3)[0], height=(0.5, 0.3)[1],
        ori=0.0, pos=(-0.7, -0.44), draggable=False, anchor='bottom-left',
        lineWidth=1.0,
        colorSpace='rgb', lineColor='white', fillColor='white',
        opacity=None, depth=0.0, interpolate=True)
    stimuli2 = visual.Rect(
        win=win, name='stimuli2',
        width=(0.5, 0.3)[0], height=(0.5, 0.3)[1],
        ori=0.0, pos=(0.7, 0.44), draggable=False, anchor='top-right',
        lineWidth=1.0,
        colorSpace='rgb', lineColor='white', fillColor='white',
    ...
    for arg in sys.argv:
        if arg.startswith('--freq1='):
            freq1 = float(arg.split('=')[1])
        if arg.startswith('--freq2='):
            freq2 = float(arg.split('=')[1])
    
    frame_rate = win.getActualFrameRate()
    max_freq = frame_rate / 2
    
    if freq1 >= max_freq or freq2 >= max_freq:
        raise ValueError(f"Frequencies must be below {max_freq}Hz (half your frame rate of {frame_rate}Hz)")
    
    frames_per_half_cycle_first = int(frame_rate / (2 * freq1))
    frames_per_half_cycle_second = int(frame_rate / (2 * freq2))
    frame_count = 0
    ...
    for thisTrial in trials:
        ...
        while continueRoutine and routineTimer.getTime() < 20.0:
            ...
            if stimuli1.status == STARTED:
                ...
                if tThisFlipGlobal > stimuli1.tStartRefresh + 20-frameTolerance:
                    # keep track of stop time/frame for later
                    stimuli1.tStop = t  # not accounting for scr refresh
                    stimuli1.tStopRefresh = tThisFlipGlobal  # on global time
            ...
            if stimuli2.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > stimuli2.tStartRefresh + 20.0-frameTolerance:
                    # keep track of stop time/frame for later
                    stimuli2.tStop = t  # not accounting for scr refresh
                    stimuli2.tStopRefresh = tThisFlipGlobal  # on global time
            ...
            cycle_position_first = frame_count % (frames_per_half_cycle_first * 2)
            cycle_position_second = frame_count % (frames_per_half_cycle_second * 2)
            
            if cycle_position_first < frames_per_half_cycle_first:
                stimuli1.opacity = 1
            else:
                stimuli1.opacity = 0
                
            if cycle_position_second < frames_per_half_cycle_second:
                stimuli2.opacity = 1
            else:
                stimuli2.opacity = 0
