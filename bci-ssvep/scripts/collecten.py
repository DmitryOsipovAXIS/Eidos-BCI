...
def _draw_dual(
	...
	screen.fill(BG)
	left_col = WHITE if left_on else DIM
	right_col = WHITE if right_on else DIM
	pygame.draw.rect(screen, left_col, lrect)
	pygame.draw.rect(screen, right_col, rrect)
...
def _start_menu(fullscreen: bool) -> dict:
	...
	while selected is None:
		...
		for k, text, _ in options:
			label = f"{k}. {text}"
			surf = fmed.render(label, True, GREY)
...
def _run_single(
	...
	cy = H // 2
	if side == "left":
		rect = pygame.Rect(int(W * 0.12), cy - bh // 2, bw, bh)
	else:
		rect = pygame.Rect(int(W * 0.53), cy - bh // 2, bw, bh)
	...
	while pygame.time.get_ticks() < end_t and running:
		...
		on = phase < 0.5
		fps = clock.get_fps()
		_draw_single(screen, rect, on, label, hz, fbig, fmed, fps)
		pygame.display.flip()
...
def main() -> None:
	...
	eeg = _run_single(args, stream, fs, side, hz, duration_s, label)
	if stream is not None:
		try:
			stream.stop_stream()
		except Exception:
